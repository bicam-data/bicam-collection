...
def convert_value(value, target_type):
    ...
    if target_type.startswith('int'):
        return int(value)
    if target_type.startswith(('float', 'numeric')):
        return float(value)
    if target_type.startswith(('timestamp', 'date')):
        try:
    ...
    async with pool.acquire() as conn:
        try:
            ...
            if total_processed != total_inserted:
                logger.info("Some rows already existed in the table or were skipped due to errors.")
