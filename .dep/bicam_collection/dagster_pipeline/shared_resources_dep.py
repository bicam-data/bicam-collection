...
class ProcessingResource(ConfigurableResource):
    ...
    _cleanup_lock: asyncio.Lock | None = None  # Prevents concurrent cleanup
    _cleanup_in_progress: bool = False  # Track cleanup state
    _cleanup_completed: bool = False  # Track cleanup completion
    ...
    async def cleanup(self):
        """Clean up all resources with proper race condition handling."""
        # Initialize cleanup lock if not already done
        if self._cleanup_lock is None:
            self._cleanup_lock = asyncio.Lock()

        # Use timeout to prevent hanging during cleanup
        try:
            async with asyncio.timeout(120):  # 2 minute timeout for cleanup
                async with self._cleanup_lock:
                    ...
                    logger.info("Starting bicam processing resources cleanup")

                    # Define cleanup tasks in proper order
                    cleanup_tasks = [
                        ("storage_manager", self._cleanup_storage_manager),
                        ("api_keys", self._cleanup_api_keys),
                        ("api_clients", self._cleanup_api_clients),
                        ("run_manager", self._cleanup_run_manager),
                        ("db_pool", self._cleanup_db_pool),
                        ("state_variables", self._cleanup_state_variables),
                    ]

                    # Execute cleanup tasks in order with individual error handling
                    ...
                    logger.info("Bicam processing resources cleanup completed")

        except asyncio.TimeoutError:
            logger.error("Overall cleanup timeout - forcing completion")
            self._cleanup_completed = True
        except Exception as e:
    ...
    async def _cleanup_storage_manager(self):
        """Clean up storage manager first to ensure data is flushed."""
    ...
    async def _cleanup_api_keys(self):
        """Clean up API keys and parallel sessions."""
        if self._system_key_manager:
            try:
                # Create a copy of keys to avoid modification during iteration
                data_types = list(self._parallel_sessions.keys())
                for data_type in data_types:
                    try:
                        await self.release_keys_for_data_type(data_type)
                    except Exception as e:
                        logger.warning(f"Error releasing keys for {data_type}: {e}")
                logger.debug("API keys released successfully")
            except Exception as e:
    ...
    async def _cleanup_run_manager(self):
    ...
    async def _cleanup_db_pool(self):
        """Clean up database pool with proper event loop handling."""
        if self._db_pool:
            try:
    ...
    async def _cleanup_state_variables(self):
        ...
        try:
            ...
            logger.debug("State variables cleared successfully")
        except Exception as e:
            logger.warning(f"Error clearing state variables: {e}")
