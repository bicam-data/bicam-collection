"""
Consolidated base database normalizer.

This module provides a unified base normalizer that can handle both Congressional and GovInfo
data types with configurable schemas and processing strategies.

FEATURES:
- Configurable schema defaults (target_schema, source_schema)
- Configurable system names for progress tracking
- Advanced batch processing with checkpoint support
- Bulk database operations with COPY support
- Phase-specific processing for resumable operations
...
from bicam_collection.libs.batch_accumulator import BatchAccumulator
from bicam_collection.libs.checkpoint import (
    HierarchicalProgressTracker,
    ProcessingPhase,
    ProcessingStage,
)
from bicam_collection.libs.record_processing_context import RecordProcessingContext
from bicam_collection.libs.run_tracking import RunMetadata, RunType

logger = logging.getLogger(__name__)


class BaseDatabaseNormalizer:
    """
    Consolidated base normalizer for all data types.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            target_schema: Target schema for staging data (auto-detected if None)
            source_schema: Source schema for raw data (auto-detected if None)
            **kwargs: Additional arguments passed to AbstractDatabaseNormalizer
        """
        # Auto-detect schemas based on system name if not provided
        if target_schema is None:
            target_schema = f"bicam_staging_{system_name}"
        if source_schema is None:
            source_schema = f"bicam_raw_{system_name}"
        ...
        try:
            ...
            self.main_list_key = self.main_config.api.list_key
        except Exception as e:
            logger.warning(f"Could not load config for {data_type_name}: {e}")
            self.main_config = None
            self.main_table_name = None
            self.main_id_field = None
            self.main_list_key = None

        # Initialize class-level batch accumulator if needed
        if BaseDatabaseNormalizer._batch_accumulator is None:
            BaseDatabaseNormalizer._batch_accumulator = BatchAccumulator(
                max_batch_size=5000,  # Increased from 1000 to 5000 for better performance
                max_memory_mb=500,  # Increased from 200 to 500 for better bulk performance
            )
    ...
    async def setup_run_tracking(
        ...
        if not run_metadata:
            run_metadata = RunMetadata(
                ...
                description=f"{self.system_name.title()} {self.data_type_name} data normalization to staging",
    ...
    def _get_related_tables_with_raw_data(self) -> list[str]:
        ...
        return self.main_config.related_tables if self.main_config else []
    ...
    async def _process_main_records(
        ...
        for record in raw_data:
            ...
            if self.progress_tracker and self.progress_tracker.should_skip_item(
                record_id, ProcessingPhase.PHASE_1, field_name="main_data"
            ):
                logger.debug(
    ...
    async def _process_nested_records(
        ...
        for record in raw_data:
            ...
            if self.progress_tracker and self.progress_tracker.should_skip_item(
                record_id, ProcessingPhase.PHASE_2, field_name="nested_data"
            ):
                logger.debug(
    ...
    def get_incomplete_items_for_resume(
        ...
        for item_id in available_item_ids:
            ...
            if not self.progress_tracker.should_skip_item(
                item_id, ProcessingPhase.PHASE_1, field_name="main_data"
            ):
                incomplete_phases["main_data"].append(item_id)

            # Check related data processing phase
            if not self.progress_tracker.should_skip_item(
                item_id, ProcessingPhase.PHASE_2, field_name="related_data"
            ):
            ...
            if not self.progress_tracker.should_skip_item(
                item_id, ProcessingPhase.PHASE_2, field_name="nested_data"
            ):
    ...
    async def process_items(
        ...
        rerun: bool = False,
        batch_size: int = 1000,  # Increased from 200 to 1000 for better performance
        max_concurrent: int = 50,  # Increased from 20 to 50 for better parallelization
        resume: bool = True,
    ) -> dict[str, Any]:
        """
        ...
        try:
            if resume and not rerun:
                ...
                # Traditional processing without checkpoints (rerun mode) - OPTIMIZED
                # Process all data in a single pass like the old version
                raw_data = await self._get_raw_data_by_ids(item_ids)
                logger.info(f"Retrieved {len(raw_data)} raw records for processing")

                # Process main records
                main_stats = await self._process_data_batch_optimized(
                    raw_data, "main", batch_size, max_concurrent
                )
                stats["main_records_processed"] = main_stats.get("records_processed", 0)
                stats["errors"] += main_stats.get("errors", 0)
                ...
                for table_suffix in related_tables:
                    try:
                        related_raw_data = await self._get_related_raw_data_by_ids(
                            item_ids, table_suffix
                        )
                        if related_raw_data:
                            related_stats = await self._process_data_batch_optimized(
                                related_raw_data,
                                table_suffix,
                                batch_size,
                                max_concurrent,
                        ...
                        logger.error(f"Error processing {table_suffix}: {e}")
                        stats["errors"] += 1
    ...
    async def flush_remaining_batches(self) -> dict[str, Any]:
        ...
        if not self._batch_accumulator or self._batch_accumulator.is_empty():
    ...
    async def _store_all_table_records_batched(
        ...
        async with BaseDatabaseNormalizer._accumulator_lock:
            accumulator = BaseDatabaseNormalizer._batch_accumulator
    ...
    async def _ensure_table_with_schema_using_conn(
        ...
        table_exists = False
        async with BaseDatabaseNormalizer._cache_lock:
            if cache_key in BaseDatabaseNormalizer._table_schema_cache:
                table_exists = True
        ...
        try:
            ...
            if not table_exists:
                ...
                if table_exists:
                    async with BaseDatabaseNormalizer._cache_lock:
                        BaseDatabaseNormalizer._table_schema_cache[cache_key] = True
    ...
    async def _get_raw_data_by_ids(self, item_ids: list[str]) -> list[dict[str, Any]]:
        """Consolidated method to get raw data by IDs."""
        if not item_ids:
            return []
    ...
    async def _get_related_raw_data_by_ids(
        ...
        """Consolidated method to get related raw data by IDs."""
        if not item_ids:
            return []
    ...
    async def _process_single_record(
        ...
        try:
            ...
            if record_type == "main":
                processing_phase = ProcessingPhase.PHASE_1
                field_name = "main_data"
            else:
                processing_phase = ProcessingPhase.PHASE_2
    ...
    async def _process_single_nested_record(
        ...
        try:
            ...
            if self.progress_tracker:
                self.progress_tracker.set_processing_phase(
                    ProcessingPhase.PHASE_2, current_field="nested_data"
                )
    ...
    def _extract_deeply_nested_lists(
        ...
        """
        Extract deeply nested lists (lists within lists) - PRESERVED LOGIC.

        This handles the recursive case where a nested field (list of dicts) contains
        another list of dicts that needs to be extracted to its own table.
        This is essential for complex hierarchical data structures.
        """
    ...
    async def _process_data_batch_optimized(
        self,
        raw_data: list[dict[str, Any]],
        record_type: str,
        batch_size: int,
        ...
        """Optimized batch processing without checkpoint overhead."""
        if not raw_data:
            return {"records_processed": 0, "errors": 0}
        ...
        for i in range(0, len(raw_data), batch_size):
            ...
            batch_stats = await self._run_concurrent_batch_optimized(
                batch,
                lambda rec, rt=record_type: self._process_single_record_optimized(
                    rec, rt
                ),
                max_concurrent,
            )
    ...
    async def _run_concurrent_batch_optimized(
        self,
        batch: list[Any],
        handler,
        max_concurrent: int,
    ...
    async def _process_single_record_optimized(
        self, raw_record: dict[str, Any], record_type: str
    ) -> dict[str, Any]:
        """Optimized single record processing without checkpoint overhead."""
        try:
            payload = raw_record.get("payload", {})
            if isinstance(payload, str):
                payload = json.loads(payload)
