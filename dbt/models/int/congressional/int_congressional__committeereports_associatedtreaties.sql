...
{%- set cols = columns_in(ref('stg_congressional__committeereports_associatedtreaties')) -%}

with base as (
  select * from {{ ref('stg_congressional__committeereports_associatedtreaties') }}
),
dedup as (
  select b.*,
         row_number() over (
           partition by
             ...
             {%- if 'treaty_id' in cols %} b.treaty_id, {%- endif %}
