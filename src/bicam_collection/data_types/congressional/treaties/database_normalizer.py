...
class TreatiesDatabaseNormalizer(CongressionalBaseDatabaseNormalizer):
    ...
    def _get_related_tables_with_raw_data(self) -> list[str]:
        """Get list of related table suffixes that have raw data tables for treaties."""
        try:
            ...
            for config in all_configs:
                ...
                if config.table_name.startswith("treaties_"):
                    table_suffix = config.table_name.replace("treaties_", "")
                    related_tables.append(table_suffix)
                else:
                    table_suffix = config.name.replace("treaties_", "")
                    related_tables.append(table_suffix)
