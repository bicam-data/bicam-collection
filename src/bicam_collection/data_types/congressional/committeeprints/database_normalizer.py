...
class CommitteeprintsDatabaseNormalizer(CongressionalBaseDatabaseNormalizer):
    ...
    def _get_related_tables_with_raw_data(self) -> list[str]:
        """Get list of related table suffixes that have raw data tables for committeeprints."""
        try:
            ...
            for config in all_configs:
                ...
                if config.table_name.startswith("committeeprints_"):
                    table_suffix = config.table_name.replace("committeeprints_", "")
                    related_tables.append(table_suffix)
                else:
                    table_suffix = config.name.replace("committeeprints_", "")
                    related_tables.append(table_suffix)
    ...
    def _process_item_record(
        ...
        if isinstance(data, dict):
            ...
            # Case 2: Direct committeeprint data (no wrapper) with committeeprints-specific validation
                ...
                logger.debug(f"Found direct committeeprint data (no wrapper) for {existing_id}")
