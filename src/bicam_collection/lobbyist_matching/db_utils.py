...
class DatabaseInterface:
            ...
            while retry_count < max_retries:
                try:
                    async with self.pool.acquire() as conn:
                        ...
                        if specific_filings:
                            filing_uuids = [
                                str(f) if isinstance(f, uuid.UUID) else f
                                for f in specific_filings
                            ]
                            query_parts.append(
                                f"AND fs.filing_uuid = ANY(${param_counter}::uuid[])"
                            )
                        ...
                        if year_range:
                            ...
                            query_parts.append(
                                f"AND f.filing_year BETWEEN ${param_counter} AND ${param_counter + 1}"
                            )
                        ...
                        current_batch_size = min(
                            batch_size, limit - total_fetched if limit else batch_size
                        )
                    ...
                    if retry_count == max_retries:
                        logging.error(
                            f"Failed to fetch sections after {max_retries} attempts"
                        )
                        raise
                    ...
                    logging.warning(
                        f"Fetch attempt {retry_count} failed, retrying in {wait_time}s: {str(e)}"
                    )
    ...
    async def get_filtered_sections_from_custom_table(
        self,
        ...
        """
        Get filing sections from custom table configuration with flexible filtering options.

        Args:
            table_config: Dictionary with keys:
        ...
        Returns:
            List of FilingSection objects matching the filter criteria
        """
        # Set defaults for column names
        table_name = table_config["table_name"]
        filing_uuid_col = table_config.get("filing_uuid_col", "filing_uuid")
        section_id_col = table_config.get("section_id_col", "section_id")
        text_col = table_config.get("text_col", "issue_text")
        year_col = table_config.get("year_col", "filing_year")
                    ...
                    async with self.pool.acquire() as conn:
                        ...
                        if random_sample:
                            query_parts = [
                                ...
                                WITH filtered_sections AS (
                                    SELECT DISTINCT
                                        {filing_uuid_col} as filing_uuid,
                                        {section_id_col} as section_id,
                                        COALESCE({text_col}, '') as text,
                                        {year_col} as filing_year
                                    FROM {table_name}
                                    WHERE {text_col} IS NOT NULL
                                    AND length({text_col}) > $1
                                """
                            ]
                        else:
                            query_parts = [
                        ...
                        if additional_where:
                            query_parts.append(f"AND ({additional_where})")
                        ...
                        if specific_filings:
                            ...
                            query_parts.append(
                                f"AND {filing_uuid_col} = ANY(${param_counter}::uuid[])"
                            )
                            params.append(filing_uuids)
                            param_counter += 1
                        ...
                        if year_range:
                            start_year, end_year = year_range
                            query_parts.append(
                                f"AND {year_col} BETWEEN ${param_counter} AND ${param_counter + 1}"
                            )
                            params.extend([start_year, end_year])
                            param_counter += 2
                        ...
                        if random_sample:
                            ...
                            query_parts.append(f"ORDER BY {section_id_col}")
                        ...
                        query = "\n".join(query_parts)
                        logging.debug(
                            f"Executing custom table query with params: {params}"
                        )
    ...
    async def bulk_insert_references(
        self,
        ...
        logging.info(
            f"Inserting {len(references)} references in batches of {batch_size}"
        )
    ...
    async def insert_unmatched_sections(
        self,
        conn: asyncpg.Connection,
        run_id: int,
