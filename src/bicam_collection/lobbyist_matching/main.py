...
class CheckpointManager:
    ...
    async def get_last_checkpoint(self, run_id: int) -> str | None:
    ...
    async def get_run_status(self, run_id: int) -> str | None:
...
async def ensure_schema_exists(pool: asyncpg.Pool, command_timeout: int = 300):
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            logging.error(
                "Timeout while creating schema, defaulting to schema already exists"
            )
            return True
    ...
    while attempt < max_retries:
        try:
            ...
            async with db.pool.acquire() as conn:
                ...
                if not total_refs:
                    raise ValueError(
                        f"No references found for run ID {extraction_run_id}"
                    )
                ...
                with tqdm(total=total_refs, desc="Copying references") as pbar:
                    while copied_refs < total_refs:
                        try:
                            ...
                            pbar.update(
                                batch_size
                                if copied_refs < total_refs
                                else total_refs - (copied_refs - batch_size)
                            )
                            ...
                            logging.warning(
                                f"Timeout occurred at {copied_refs} refs, continuing..."
                            )
                            continue
            ...
            if attempt < max_retries:
                logging.warning(
                    f"Database connection failed, retrying in {retry_delay} seconds... (Attempt {attempt}/{max_retries})"
                )
...
async def process_filings(
    ...
    Args:
        ...
        table_config: Optional custom table configuration with keys:
            - 'table_name': Full table name (e.g., 'my_schema.my_table')
            - 'filing_uuid_col': Column name for filing UUID (default: 'filing_uuid')
            - 'section_id_col': Column name for section ID (default: 'section_id')
            - 'text_col': Column name for text content (default: 'issue_text')
            - 'year_col': Column name for filing year (default: 'filing_year')
            - 'additional_where': Optional additional WHERE conditions
    ...
    try:
        ...
        # Get sections based on filters - use custom table if provided
        if table_config:
            logging.info(f"Using custom table: {table_config['table_name']}")
            sections = await db.get_filtered_sections_from_custom_table(
            ...
            logging.info("Using default tables: relational___lda.filing_sections*")
        ...
        logging.info(
            f"Processing {len(sections)} sections from {total_filings} filings"
        )
...
async def create_db_pool_with_retry(
    db_config: dict[str, str], max_retries: int = 5, retry_delay: int = 5
    ...
    while attempt < max_retries:
        try:
            ...
            if attempt < max_retries:
                logging.warning(
                    f"Database connection failed (attempt {attempt}/{max_retries}), retrying in {retry_delay} seconds..."
                )
                await asyncio.sleep(retry_delay)
...
async def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"bill_extraction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            ),
        ],
    )

    parser = argparse.ArgumentParser(
        description="Extract bill references from filing sections"
    )
    parser.add_argument(
        "--sample-size",
        type=int,
    ...
    parser.add_argument(
        "--year-start", type=int, default=None, help="Start year for filtering filings"
    )
    parser.add_argument(
        "--year-end", type=int, default=None, help="End year for filtering filings"
    )
    parser.add_argument(
        "--match-only",
        type=int,
    ...
    parser.add_argument(
        "--post-process", type=int, help="Run ID to perform all post-processing steps"
    )
    parser.add_argument(
        "--custom-table",
        type=str,
        help="Custom table name to process (e.g., 'my_schema.my_table')",
    )
    parser.add_argument(
        "--filing-uuid-col",
        type=str,
        default="filing_uuid",
        help="Column name for filing UUID",
    )
    parser.add_argument(
        "--section-id-col",
        type=str,
        default="section_id",
        help="Column name for section ID",
    )
    parser.add_argument(
        "--text-col",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="filing_year",
        help="Column name for filing year",
    )
    parser.add_argument(
        "--additional-where", type=str, help="Additional WHERE clause conditions"
    )
    ...
    try:
        ...
        if args.post_process is not None:
            try:
                ...
                logging.info(
                    f"Post-processing completed for run ID: {args.post_process}"
                )
            except Exception as e:
            ...
            if args.custom_table:
                table_config = {
                    "table_name": args.custom_table,
                    "filing_uuid_col": args.filing_uuid_col,
                    "section_id_col": args.section_id_col,
                    "text_col": args.text_col,
                    "year_col": args.year_col,
                }
                if args.additional_where:
                    table_config["additional_where"] = args.additional_where
