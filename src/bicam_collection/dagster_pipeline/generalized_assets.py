...
def create_raw_data_asset(data_type: str):
    ...
    def raw_data_asset(
        ...
        The asset is synchronous but can call async methods by creating
        ...
        # Execute async operations in a separate thread to avoid event loop conflicts
        def run_async_operations():
            ...
            try:
                return loop.run_until_complete(_async_fetch_processing())
            finally:
                loop.close()

        async def _async_fetch_processing():
            # Get or create async resources
            ...
            try:
                # Get API clients for this data type
                key_manager = processing_resource.get_system_key_manager()
                keys = key_manager.assign_keys_for_data_type(data_type)
                ...
                if explicit_dates_provided:
                    ...
                    dagster_logger.info(
                        "Explicit dates override incremental processing"
                    )
                    ...
                    last_processed = await client.access_last_processed_date(data_type)
                    if last_processed:
                ...
                run_type = getattr(
                    RunType,
                    f"SCRAPER_{data_source.upper()}",
                    RunType.SCRAPER_CONGRESSIONAL,
                )

                run_metadata = RunMetadata(
                    run_id="",  # Will be set by run manager
                    run_type=run_type,
                ...
                # Execute the actual fetcher processing
                dagster_logger.info(f"Starting fetcher execution for {data_type}")
                stats = await fetcher.process_items(**processing_params)

                # Update last processed date only if using incremental system
                if (
                    use_incremental_dates
                    and processing_resource.incremental
                    ...
                    success = await client.update_last_processed_date(
                    ...
                    if success:
                        dagster_logger.info(
                            ...
                            + (
                                f" with total_count: {total_count}"
                                if total_count
                                else ""
                            )
                        )
                ...
                dagster_logger.info(f"Completed raw data fetching for {data_type}")
                return {
                    "stage": "raw_data",
                    "data_type": data_type,
                    "data_source": data_source,
        ...
        try:
            ...
            dagster_logger.error(f"Error in raw data asset for {data_type}: {e}")
            raise
...
def create_staging_data_asset(data_type: str):
    ...
    def staging_data_asset(
        ...
        All async setup is done in the setup phase, so this asset can be
        purely synchronous.
        """
        dagster_logger = get_dagster_logger()
        ...
        def run_async_operations():
            ...
            try:
                return loop.run_until_complete(_async_staging_processing())
            finally:
                loop.close()

        async def _async_staging_processing():
            # Get or create async resources
            db_pool = await processing_resource.get_db_pool()
            checkpoint_manager = processing_resource.get_checkpoint_manager()
            run_manager = await processing_resource.get_run_manager(db_pool)
            ...
            try:
                ...
                # Set processing type for this phase
                processing_resource.set_processing_type("normalizer")
                ...
                run_metadata = RunMetadata(
                    run_id="",  # Will be set by run manager
                    run_type=run_type,
                    system_name=f"{data_type}_normalizer_dagster",
                    description=f"Dagster staging data normalization for {data_type} ({data_source})",
                    data_types=[data_type],
                    parameters={
                        "data_type": data_type,
                ...
                # Execute the actual normalizer processing
                dagster_logger.info(
                    f"Starting normalizer execution for {data_type} with {len(item_ids)} items"
                )
                ...
                dagster_logger.info(
                    f"Completed staging data normalization for {data_type}"
                )
                return {
                    "stage": "staging_data",
                    "data_type": data_type,
                ...
                dagster_logger.error(
                    f"Staging normalization failed for {data_type}: {e}"
                )
                if "run_id" in locals():
                    await run_manager.fail_run(run_id, str(e))
                raise
        ...
        try:
            ...
            dagster_logger.error(f"Error in staging data asset for {data_type}: {e}")
            raise
...
def create_production_data_asset(data_type: str):
    ...
    def production_data_asset(
        ...
        def run_async_operations():
            ...
            try:
                return loop.run_until_complete(_async_production_processing())
            finally:
                loop.close()

        async def _async_production_processing():
            # Get or create async resources
            db_pool = await processing_resource.get_db_pool()
            checkpoint_manager = processing_resource.get_checkpoint_manager()
            run_manager = await processing_resource.get_run_manager(db_pool)
            ...
            try:
                ...
                # Set processing type for this phase
                processing_resource.set_processing_type("cleaner")
                ...
                # Get item IDs from staging data
                item_ids = await _get_item_ids_from_staging_data(
                    db_pool, data_type, data_source
                )

                if not item_ids:
                    dagster_logger.warning(f"No {data_type} IDs found in staging data")
                    return {
                        ...
                        "stats": {"processed": 0, "failed": 0, "skipped": 0},
                        "message": "No staging data available",
                    }
                ...
                run_metadata = RunMetadata(
                    run_id="",  # Will be set by run manager
                    run_type=run_type,
                    system_name=f"{data_type}_cleaner_dagster",
                    description=f"Dagster production data cleaning for {data_type} ({data_source})",
                    data_types=[data_type],
                    parameters={
                        "data_type": data_type,
                ...
                # Execute the actual cleaner processing
                dagster_logger.info(
                    f"Starting cleaner execution for {data_type} with {len(item_ids)} items"
                )
                ...
                dagster_logger.info(
                    f"Completed production data cleaning for {data_type}"
                )
                return {
                    "stage": "production_data",
                    "data_type": data_type,
        ...
        try:
            ...
            dagster_logger.error(f"Error in production data asset for {data_type}: {e}")
            raise
