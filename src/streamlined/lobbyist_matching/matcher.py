...
class BillInfo:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.official_titles, list):
            object.__setattr__(
                self, "official_titles", tuple(self.official_titles or ())
            )
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.bill_type == other.bill_type
            and self.bill_number == other.bill_number
        )
...
class AppropriationsTrie:
    ...
    def __init__(self):
        self.bills: dict[
            tuple[str, str], AppropriationsBill
        ] = {}  # (bill_type, number) -> AppropriationsBill
        self.title_to_bills: dict[str, set[tuple[str, str]]] = defaultdict(
            set
        )  # normalized title -> set of (type, number)
    ...
    def add_bill(self, bill: BillInfo) -> None:
        ...
        all_titles = tuple(
            filter(None, tuple(bill.titles or ()) + tuple(bill.official_titles or ()))
        )
    ...
    def find_matching_bills(
        self, title: str
    ) -> list[tuple[AppropriationsBill, str, float]]:
...
class ReferenceMatcher:
    ...
    def __init__(self, bill_trie: BillTrie, logger: MatchingLogger = None):
        ...
        for congress in self.bill_trie.congress_nodes:
            for bill_type in self.bill_trie.congress_nodes[congress]:
                for bill_number, bill in self.bill_trie.congress_nodes[congress][
                    bill_type
                ].items():
    ...
    def _match_bill_number(self, reference: dict) -> MatchResult:
        ...
        # Number only match
        corpus_title = (
            bill.official_titles[0]
            if bill.official_titles
            else bill.titles[0]
            if bill.titles
            else None
        )
    ...
    def _match_title_only(self, reference: dict) -> MatchResult | None:
        ...
        for title_variant in titles_to_check:
            ...
            for bill in corpus_title_set:
                # Optional filter by chamber/bill_type if provided
                desired_type = reference.get("bill_type")
                if desired_type and bill.bill_type != desired_type:
                    continue
                ...
                for bill_title in bill.official_titles:
                    ...
                    word_overlap = len(title_words & corpus_words) / max(
                        len(title_words), len(corpus_words)
                    )
    ...
    def _match_with_title(self, reference: dict, bill: BillInfo) -> MatchResult:
        ...
        logging.debug(
            f"Bill info: Congress {bill.congress}, {bill.bill_type}{bill.bill_number}"
        )
        ...
        # Case-insensitive check for formal titles
        is_formal_title = any(
            re.match(pattern, extracted_title, re.I)
            for pattern in formal_title_patterns
        )
        ...
        for i, title in enumerate(all_titles, 1):
            ...
            corpus_title_to_check = (
                year_pattern.sub("", title).strip() if not has_year else title
            )
        ...
        match_type = (
            "high_confidence_match"
            if best_score >= self.wrong_title_threshold
            else "wrong_title"
        )
            ...
            for (filing_uuid, section_id), refs in sections_dict.items():
                ...
                while i < len(refs):
                    ...
                    if ref["reference_type"] == "number":
                        ...
                        while (
                            next_pos < len(refs)
                            and refs[next_pos]["reference_type"] == "number"
                            and refs[next_pos]["start_position"]
                            - companion_numbers[-1]["end_position"]
                            <= 5
                        ):
                        ...
                        if (
                            next_pos < len(refs)
                            and refs[next_pos]["reference_type"] == "title"
                            and refs[next_pos]["start_position"]
                            - companion_numbers[-1]["end_position"]
                            <= 10
                        ):
                            ...
                            logging.debug(
                                f"Combining references: {[r['reference_id'] for r in companion_numbers]} + {title_ref['reference_id']}"
                            )
                            ...
                            for num_ref in companion_numbers:
                                if num_ref["reference_id"] is not None:
                                    ...
                                    logging.warning(
                                        f"Number reference with None reference_id encountered: {num_ref}"
                                    )
                            ...
                            if title_ref["reference_id"] is not None:
                                ...
                                logging.warning(
                                    f"Title reference with None reference_id encountered: {title_ref}"
                                )
                            ...
                            for num_ref in companion_numbers:
                                combined_ref = (
                                    ...
                                    num_ref.get(
                                        "full_match"
                                    ),  # Include full_match if available
                                )
                                all_updates.append(combined_ref)
                                logging.debug(
                                    f"Assigning new reference_id {next_ref_id} to combined reference."
                                )
                                next_ref_id += 1
            ...
            if all_updates:
                async with conn.transaction():
                    ...
                    for idx in range(0, len(all_updates), chunk_size):
                        ...
                        logging.debug(
                            f"Inserting chunk {idx // chunk_size + 1}: {len(chunk)} records."
                        )
                    ...
                    await conn.execute(
                        ...
                        AND run_id = $2
                    """,
                        list(all_ref_ids_to_update),
                        run_id,
                    )

            logging.info(
                f"Updated {len(all_ref_ids_to_update)} original references' types."
            )

        logging.info(
            f"Processed {len(sections_dict)} sections, created {len(all_updates)} combinations."
        )
...
def normalize_title(title: str) -> str:
    ...
    title = re.sub(
        r",\s*(?:19|20)\d{2}(?:\s*$|\s*(?:and|through)\s*(?:19|20)\d{2}\s*$)", "", title
    )
    ...
    async with pool.acquire() as conn:
        ...
        for row in tqdm(rows, desc="Building bill trie"):
            try:
                if (
                    not row["congress"]
                    or not row["bill_type"]
                    or not row["bill_number"]
                ):
                    continue
                        ...
                        while retry_count < max_retries:
                            try:
                                async with conn.transaction():
                                    ...
                                    for ref in refs:
                                        try:
                                            match = self.matcher.match_reference(
                                                dict(ref)
                                            )  # Use self.matcher here
                                            if match:
                                                matches.append(
                                                    (
                                                        run_id,
                                                        ...
                                                        ref[
                                                            "title"
                                                        ],  # extract from original reference
                                                        ref[
                                                            "bill_id"
                                                        ],  # extract from original reference
                                                        ref.get(
                                            ...
                                            logging.error(
                                                f"Error matching reference {ref['reference_id']}: {str(e)}"
                                            )
                                        ...
                                        while store_retry_count < max_retries:
                                            try:
                                                ...
                                                if store_retry_count == max_retries:
                                                    logging.error(
                                                        f"Error storing matches after {max_retries} attempts: {str(e)}"
                                                    )
                                                    raise
                                                logging.warning(
                                                    f"Error storing matches (attempt {store_retry_count}/{max_retries}): {str(e)}"
                                                )
                                                await asyncio.sleep(
                                                    store_retry_count * 2
                                                )
                                ...
                                if retry_count == max_retries:
                                    ...
                                    raise
                                logging.warning(
                                    f"Operation timed out (attempt {retry_count}/{max_retries}), retrying..."
                                )
                                await asyncio.sleep(
                                    retry_count * 5
                                )  # Exponential backoff
                                ...
                                if retry_count == max_retries:
                                    ...
                                    raise
                                logging.warning(
                                    f"Operation failed (attempt {retry_count}/{max_retries}): {str(e)}"
                                )
...
def process_batch(
    ...
    for ref in refs:
        try:
            ...
            logging.error(
                f"Error processing reference {ref.get('reference_id')}: {str(e)}"
            )
            continue
    return results
...
async def store_matches(
    conn: asyncpg.Pool, run_id: int, matches: list[MatchResult | None]
) -> None:
    ...
    valid_matches = [
        ...
        and hasattr(match, "reference_id")
        and match.reference_id is not None
    ]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:  # Last attempt
                    ...
                    logging.warning(
                        f"Error storing matches (attempt {attempt + 1}/{max_retries}): {str(e)}"
                    )
                    await asyncio.sleep(
...
def normalize_appropriations_title(title: str) -> str:
    ...
    title = re.sub(
        r"\b(?:for\s+)?(?:fiscal\s+year|FY)\s*(?:19|20)\d{2}\b", "", title, flags=re.I
    )
    ...
    title = re.sub(
        r"\s*(?:bill|act|appropriations|approps?)\s*$", "", title, flags=re.I
    )
