...
BILL_TYPE_PATTERNS = {
    ...
    # Support hyphen between type and number (e.g., HR-123)
    "nid": r"(?:-?\s*\d+)(?:-\d+)?[x_]*",
}

# Core pattern for matching bill numbers
BILL_NUM_PATTERN = re.compile(
    rf"\b(({BILL_TYPE_PATTERNS['chamber']})\s*(?::|\.|\s|-)\s*({BILL_TYPE_PATTERNS['res_type']})?\s*"
    rf"({BILL_TYPE_PATTERNS['leg_type']})?(?-i:s)?\s*(?::|\.|\s|-)*)\s*({BILL_TYPE_PATTERNS['nid']})",
    re.I | re.VERBOSE,
)
    ...
    for segment in segments:
        ...
        for ref in bill_refs + law_refs:
            ref_contexts = extract_reference_context(
                segment, ref.start(), ref.end(), "bill" if ref in bill_refs else "law"
            )
...
def extract_reference_context(
    text: str, start: int, end: int, ref_type: str
...
def process_section_for_titles(text: str) -> list[dict[str, Any]]:
    ...
    # Add new pattern for "A bill to..." titles (case-insensitive)
    bill_to_pattern = re.compile(
        ...
        \s*[,;]\s*                                      # Comma or semicolon separator
        (?P<title>
            [Aa]\s+bill\s+to\s+                        # Starts with "A bill to" (case-insensitive)
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            # Expand comma-separated lists inheriting the most recent explicit bill type
            # Examples:
            #  - "S 4128, 7195, 6780" => s4128, s7195, s6780
            #  - "S.J.Res.1,H.J.Res.1,7,10,21,45" => sjres1, hjres1, hjres7, hjres10, hjres21, hjres45
            tokens = re.split(r"\s*,\s*", bills_text)
            last_type = None
            for tok in tokens:
                m = BILL_NUM_PATTERN.search(tok)
                if m and len(m.groups()) >= 5:
                    chamber, res_type, leg_type = m.groups()[1:4]
                    bt = standardize_bill_type(chamber, res_type, leg_type)
                    if bt:
                        last_type = bt.lower()
                        # also add the matched explicit bill if not already captured by bill_refs loop
                        num = re.sub(r"\D", "", m.groups()[-1])
                        all_matches.append(
                            {
                                "type": "bill",
                                "start": match.start(),
                                ...
                                "bill_type": last_type,
                                "bill_number": num,
                                "bill_id": f"{last_type}{num}",
                                "text": m.group(0).strip(),
                                "reference_type": "bill",
                            }
                        )
                else:
                    # Number-only token inherits last explicit type
                    num_only = re.fullmatch(r"\d{1,6}", tok)
                    if num_only and last_type:
                        num = num_only.group(0)
                        all_matches.append(
                            {
                                "type": "bill",
                                "start": match.start(),
            ...
            for bill_ref in bill_refs:
                try:
                    ...
                    if bill_type:
                        raw_num = bill_ref.groups()[-1]
                        bill_number = re.sub(r"\D", "", raw_num)
                        ...
                        title = re.split(
                            r"\s*-\s*(?:all\s+provisions|.*?provisions)", title
                        )[0].strip()
                        # Remove "Reconciliation Bill" and similar descriptors
                        title = re.sub(
                            r"\s*\((?:Reconciliation\s+Bill|[^)]*?\s+Bill)\)",
                            "",
                            title,
                            flags=re.I,
                        )
...
def identify_companion_groups(
    ...
    while i < len(contexts):
        ...
        while j < len(contexts):
            ...
            if len(between_text.strip()) <= 50 and not re.search(
                r"\.|\n", between_text
            ):
...
def find_title_before(text: str) -> tuple[str, int, int] | None:
    ...
    for ending_word in TITLE_ENDING_WORDS:
        ...
        if match:
            return (match.group("title"), match.start(), match.end())
    return None
...
def find_title_after(text: str) -> tuple[str, int, int] | None:
    ...
    for ending_word in TITLE_ENDING_WORDS:
        ...
        match = re.search(pattern, text, re.VERBOSE | re.I)
        if match:
            return (match.group("title"), match.start("title"), match.end("title"))
    return None


def find_and_clean_titles(
    text: str, filing_year: int | None = None
) -> list[dict[str, Any]]:
    """
...
def add_reference_with_title(
    ...
    if cleaned_title:
        ...
        start = min(
            context.start,
            context.start - len(title_text) if title_before else context.start,
        )
        end = max(
            context.end,
            context.end + len(title_text) if not title_before else context.end,
        )
...
def detect_congress(
    text: str, matches: list[dict], filing_year: int
) -> dict[int, dict]:
    """
    ...
    majority_congress = (
        max(explicit_matches, key=explicit_matches.count) if explicit_matches else None
    )
    ...
    for match in matches:
        ...
        logging.info(
            f"Match data: title={title_text}, text={match.get('text', '')}"
        )  # Debug both fields

        # Priority 1: Special prefixes indicate current congress
        if title_text and re.match(
            r"^(?:To |A bill to |A resolution )", title_text, re.I
        ):
        ...
        if title_text and re.search(
            r"\b(?:2[0-1]|1\d)(?:st|nd|rd|th)\b(?:\s+[Cc]entury\b)?", title_text
        ):
            congress_info[match_start] = {
                "number": year_to_congress(filing_year),
                "source": "filing_year",
        ...
        if title_text:
            ...
            for i, pattern in enumerate(year_patterns):
                ...
                if year_match:
                    logging.info(
                        f"Pattern {i} matched! Match groups: {year_match.groups()}"
                    )
                    ...
                    if abs(congress_num - filing_year_congress) <= 3:
                        ...
                        year_found = True
                        logging.info(
                            f"Using year {full_year} -> congress {congress_num}"
                        )
                        break
                else:
...
def process_single_section(
    ...
    try:
        logging.debug(
            f"Starting processing for section {section.section_id} (length: {len(text)})"
        )
        ...
        logging.debug(
            f"Found {len(title_matches)} initial matches in section {section.section_id}"
        )

        # Process matches
        for match in title_matches:
            match.update(
                {"filing_uuid": section.filing_uuid, "section_id": section.section_id}
            )
            all_matches.append(match)

        # Add congress information
        logging.debug(
            f"Detecting congress numbers for {len(all_matches)} matches in section {section.section_id}"
        )
        ...
        logging.info(
            f"Section {section.section_id}: Found {len(final_results)} matches "
...
def standardize_bill_type(
    chamber: str, res_type: str | None, leg_type: str | None
) -> str | None:
    """
...
def clean_title(
    ...
    ending_pattern = rf"\b({valid_endings}){year_suffix}"
    ...
    if year_match:
        year = (
            int("20" + year_match.group(1))
            if int(year_match.group(1)) < 50
            else int("19" + year_match.group(1))
        )
        if filing_year:
...
def find_standalone_titles(text: str) -> list[dict[str, Any]]:
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            title_text = match.group("title").strip()
            if title_text and not is_part_of_nested_title(
...
def validate_title(title: str, allow_acronyms: bool = False) -> bool:
    ...
    if allow_acronyms and re.match(
        r"^[A-Z]{2,}(?:\s*[-\']\s*[A-Z]+)*\s+(?:"
        + "|".join(TITLE_ENDING_WORDS)
        + r")\b",
        title,
    ...
    for ending_word in TITLE_ENDING_WORDS:
        ...
        if title.endswith(ending_word) or re.search(
            rf"{ending_word}\s*(?:of|,)\s*(?:19|20)\d{{2}}$", title
        ):
            valid_ending = True
            break
...
def extract_bill_range(
    start_num: str, end_num: str, bill_type: str
) -> list[BillNumber]:
    """
    ...
    try:
        ...
        if end < start or (end - start) > 100:  # Sanity check
            return []
        return [BillNumber(str(i), i == start, i == end) for i in range(start, end + 1)]
    except ValueError:
        return []
                    ...
                    if bill_numbers:
                        return {
                            ...
                            "bill_number": bill_numbers[
                                0
                            ].number,  # Use first number as primary
