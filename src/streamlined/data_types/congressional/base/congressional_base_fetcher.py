...
class CongressionalBaseFetcher(BaseFetcher):
    ...
    async def _get_source_doc_id(
        ...
        if table.endswith("_list_raw"):
            # Check if _extract_preliminary_item_id is async
            result = self._extract_preliminary_item_id(item)
            ...
            # Check if extract_item_id is async
            result = self.extract_item_id(item)
            if hasattr(result, "__await__"):
                return await result
            return result
