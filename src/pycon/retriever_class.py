...
from pycon.models import ErrorResult, Result
...
class Retriever(ABC):
    ...
    def _parse_data(self):
        for key, value in self.data.items():
            ...
            setattr(
                self,
                f"_{key}" if not key.startswith("_") or key != "data" else key,
                value,
            )
                ...
                try:
                    ...
                    if isinstance(response, ErrorResult):
                        self._adapter._logger.error(
                            f"Error retrieving items: {response.error_message}"
                        )
                        await self._write_error_to_database(
                            url, response.error_message, "congressional"
                        )
                        yield response
                    ...
                    if cls:
                        processed_items = await self._process_items(
                            cls, items, verbose, _id_package_init, response.pagination
                        )
                    else:
                    ...
                    self._adapter._logger.error(
                        f"Unexpected error: {error.error_message}"
                    )
                    yield error
    ...
    async def _get_granules(
        ...
        while url:
            ...
            try:
                ...
                if isinstance(response, ErrorResult):
                    self._adapter._logger.error(
                        f"Error retrieving granules: {response.error_message}"
                    )
                ...
                for granule_data in granules:
                    if verbose:
                        ...
                        if granule_link:
                            ...
                            granule_response = await self._adapter.retrieve(
                                granule_url_with_key, override=True
                            )
                ...
                error = ErrorResult(url=url, error_message=str(e))
                self._adapter._logger.error(
                    f"Unexpected error: {error.error_message}", exc_info=True
                )
                yield error
                await self._write_error_to_database(url, str(e), "govinfo")
                break

    async def _write_error_to_database(
        self, url: str, error: str, db: str = "congressional"
    ):
        if not self._adapter.db_pool:
            self._adapter._logger.error(
                "Database pool not provided. Cannot write error to database."
            )
            return
    ...
    def _extract_items(self, data: dict[str, Any]) -> list[Any]:
        filtered_data = {
            k: v for k, v in data.items() if k not in ["request", "pagination"]
        }
    ...
    async def _process_items(
        ...
        for item in items:
            if isinstance(item, dict):
                if verbose:
                    if "url" in item:
                        response: Result = await self._adapter.retrieve(
                            item["url"], override=True
                        )
                    elif "granuleLink" in item:
                        granule_url_with_key = (
                            item["granuleLink"]
                            + f"?api_key={self._adapter.current_api_key}"
                        )
                        response: Result = await self._adapter.retrieve(
                            granule_url_with_key, override=True
                        )
