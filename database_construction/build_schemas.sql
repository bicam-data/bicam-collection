...
CREATE TABLE IF NOT EXISTS congressional.bills_sponsors(
    bill_id TEXT,
    sponsor_bioguide_id TEXT,
    PRIMARY KEY (bill_id, sponsor_bioguide_id)
    );

CREATE TABLE IF NOT EXISTS congressional.bills_committeereports(
    bill_id TEXT,
    report_id TEXT,
...
CREATE TABLE IF NOT EXISTS congressional.bills_actions(
    ...
    calendar TEXT,
    calendar_number TEXT,
    UNIQUE (bill_id, action_code, action_date, text)
    );

CREATE TABLE IF NOT EXISTS congressional.bills_actions_committee_codes(
    bill_id TEXT NOT NULL,
    action_code TEXT,
    action_date TIMESTAMP WITH TIME ZONE,
    text TEXT,
...
CREATE TABLE IF NOT EXISTS congressional.bills_actions_recorded_votes(
    ...
    session_number INTEGER,
    url TEXT,
    UNIQUE (bill_id, action_code, action_date, text, url)
    );

CREATE TABLE IF NOT EXISTS congressional.bills_committeeactivities(
    bill_id TEXT NOT NULL,
    ...
    subcommittee_activity_name TEXT,
    subcommittee_activity_date TIMESTAMP WITH TIME ZONE,
    UNIQUE (bill_id, committee_code, activity_date, subcommittee_code, subcommittee_activity_date)
    );
...
CREATE TABLE IF NOT EXISTS congressional.bills_billrelations(
    bill_id TEXT,
    relatedbill_id TEXT,
    relationship_identified_by_1 TEXT,
    relationship_type_1 TEXT,
    ...
    relationship_identified_by_3 TEXT,
    relationship_type_3 TEXT,
    PRIMARY KEY (bill_id, relatedbill_id)
    );
...
CREATE TABLE IF NOT EXISTS congressional.bills_titles(
    ...
    chamber_name TEXT,
    title_type_code TEXT,
    UNIQUE (bill_id, title, title_type)
    );
...
CREATE TABLE IF NOT EXISTS congressional.bills_summaries(
    ...
    version_code TEXT,
    updated_at TIMESTAMP WITH TIME ZONE,
    UNIQUE (bill_id, action_date, action_desc)
    );
...
CREATE TABLE IF NOT EXISTS congressional.amendments_sponsor_bioguide_ids(
    amendment_id TEXT,
    sponsor_bioguide_id TEXT,
    PRIMARY KEY (amendment_id, sponsor_bioguide_id)
    );
...
CREATE TABLE IF NOT EXISTS congressional.amendments_actions(
    ...
    source_system TEXT,
    source_system_code TEXT,
    UNIQUE (amendment_id, action_code, action_date, text)
    );
...
CREATE TABLE IF NOT EXISTS congressional.amendments_actions_recorded_votes(
    ...
    session_number INTEGER,
    url TEXT,
    UNIQUE (amendment_id, action_code, action_date, text, url)
    );
...
CREATE TABLE IF NOT EXISTS congressional.members_terms(
    ...
    party_code TEXT,
    party_name TEXT,
    UNIQUE (bioguide_id, member_type, start_year, end_year)
    );
...
CREATE TABLE IF NOT EXISTS congressional.members_leadership_roles(
    ...
    congress INTEGER,
    is_current BOOLEAN,
    UNIQUE (bioguide_id, type, congress)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committees_subcommittee_codes(
    committee_code TEXT NOT NULL,
    subcommittee_codes TEXT,
    UNIQUE (committee_code, subcommittee_codes)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committees_history(
    ...
    nara_id TEXT,
    updated_at TIMESTAMP WITH TIME ZONE,
    UNIQUE (committee_code, official_name, start_date, end_date)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committees_relatedreports(
    committee_code TEXT,
    report_id TEXT,
...
CREATE TABLE IF NOT EXISTS congressional.committeereports_associated_bill_ids(
    report_id TEXT,
    associated_bill_ids TEXT,
    PRIMARY KEY (report_id, associated_bill_ids)
    );

CREATE TABLE IF NOT EXISTS congressional.committeereports_associated_treaty_ids(
    report_id TEXT,
    associated_treaty_ids TEXT,
    PRIMARY KEY (report_id, associated_treaty_ids)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committeereports_texts(
    ...
    xml_is_errata BOOLEAN,
    url TEXT,
    PRIMARY KEY (report_id, date, type)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committeeprints_associated_bill_ids(
    print_id TEXT,
    associated_bill_ids TEXT,
    PRIMARY KEY (print_id, associated_bill_ids)
    );

CREATE TABLE IF NOT EXISTS congressional.committeeprints_committee_codes(
    print_id TEXT,
    committee_codes TEXT,
    PRIMARY KEY (print_id, committee_codes)
    );
...
CREATE TABLE IF NOT EXISTS congressional.committeemeetings_meeting_documents(
    ...
    xml TEXT,
    url TEXT,
    UNIQUE (meeting_id, name, document_type, pdf, formatted_text, html, xml, url)
    );

CREATE TABLE IF NOT EXISTS congressional.committeemeetings_hearing_jacketnumbers(
    meeting_id TEXT,
    hearing_jacketnumbers TEXT,
    PRIMARY KEY (meeting_id, hearing_jacketnumbers)
    );

CREATE TABLE IF NOT EXISTS congressional.committeemeetings_associated_bill_ids(
    meeting_id TEXT,
    associated_bill_ids TEXT,
    PRIMARY KEY (meeting_id, associated_bill_ids)
    );

CREATE TABLE IF NOT EXISTS congressional.committeemeetings_associated_treaty_ids(
    meeting_id TEXT,
    associated_treaty_ids TEXT,
    PRIMARY KEY (meeting_id, associated_treaty_ids)
    );

CREATE TABLE IF NOT EXISTS congressional.committeemeetings_associated_nomination_ids(
    meeting_id TEXT,
    associated_nomination_ids TEXT,
    PRIMARY KEY (meeting_id, associated_nomination_ids)
    );

CREATE TABLE IF NOT EXISTS congressional.hearings(
    jacket_number TEXT PRIMARY KEY,
    loc_id TEXT,
    title TEXT,
    citation TEXT,
    congress INTEGER,
...
CREATE TABLE IF NOT EXISTS congressional.hearings_committee_codes(
    jacket_number TEXT,
    committee_codes TEXT,
    PRIMARY KEY (jacket_number, committee_codes)
    );

CREATE TABLE IF NOT EXISTS congressional.hearings_dates(
    jacket_number TEXT,
    dates TEXT,
    PRIMARY KEY (jacket_number, dates)
    );

CREATE TABLE IF NOT EXISTS congressional.hearings_formats(
...
CREATE TABLE IF NOT EXISTS congressional.nominations_positions(
    ...
    intro_text TEXT,
    nominee_count INTEGER,
    UNIQUE (nomination_id, ordinal, position_title, organization, intro_text)
    );
...
CREATE TABLE IF NOT EXISTS congressional.nominations_actions(
    ...
    text TEXT,
    type TEXT,
    UNIQUE (nomination_id, action_code, action_date, text)
    );

CREATE TABLE IF NOT EXISTS congressional.nominations_actions_committee_codes(
    nomination_id TEXT NOT NULL,
    action_code TEXT,
    action_date TIMESTAMP WITH TIME ZONE,
    text TEXT,
    committee_codes TEXT NOT NULL,
    UNIQUE (nomination_id, action_code, action_date, text, committee_codes)
    );
...
CREATE TABLE IF NOT EXISTS congressional.nominations_committeeactivities(
    ...
    subcommittee_activity_name TEXT,
    subcommittee_activity_date TIMESTAMP WITH TIME ZONE,
    UNIQUE (nomination_id, committee_code, activity_date, subcommittee_code, subcommittee_activity_date)
    );
...
CREATE TABLE IF NOT EXISTS congressional.nominations_nominees(
    ...
    predecessor_name TEXT,
    corps_code TEXT,
    UNIQUE (nomination_id, ordinal, first_name, middle_name, last_name, effective_date)
    );

CREATE TABLE IF NOT EXISTS congressional.nominations_hearings(
    nomination_id TEXT,
    hearing_jacketnumber TEXT,
    errata_number TEXT,
    PRIMARY KEY (nomination_id, hearing_jacketnumber)
    );
...
CREATE TABLE IF NOT EXISTS congressional.treaties_index_terms(
    treaty_id TEXT,
    index_terms TEXT,
    PRIMARY KEY (treaty_id, index_terms)
    );

CREATE TABLE IF NOT EXISTS congressional.treaties_country_parties(
    treaty_id TEXT,
    country_parties TEXT,
    PRIMARY KEY (treaty_id, country_parties)
    );
...
CREATE TABLE IF NOT EXISTS congressional.treaties_actions_committee_codes(
    treaty_id TEXT NOT NULL,
    action_code TEXT,
    action_date TIMESTAMP WITH TIME ZONE,
    text TEXT,
...
CREATE TABLE IF NOT EXISTS congressional.congresses_sessions(
    ...
    end_date TIMESTAMP WITH TIME ZONE,
    type TEXT,
    PRIMARY KEY (number, session_number, chamber)
    );
