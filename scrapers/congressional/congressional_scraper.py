...
#         # Start by assigning 2 keys to each data type to switch off between
...
#         data types to those still being processed, allowing for more efficient
#         scraping
...
class ETLCoordinator:
    """
    Orchestrates the Extract, Transform, Load (ETL) process for Congressional data.
    The most confusing part of this script, so all documentation is extensive.
    
    This coordinator manages the end-to-end ETL pipeline, implementing:
    1. Concurrent data extraction using multiple API keys
    2. Hierarchical data processing (main entities, nested fields (so data types that exist within the object itself), and related entities (data types that require another endpoint))
    3. Stateful processing with resume capability
    4. Error handling and retry mechanisms
    5. Progress tracking and reporting
    
    The class follows a hierarchical processing pattern:
    - Main data types (bills, committees, etc.)
        ↳ Nested fields within each item
        ↳ Related entities for each item
            ↳ Nested fields within each related entity (secondary related entities)
    
    Key Features:
    - Distributed API key management for optimal throughput
    - Automatic retry and backoff mechanisms
    - Progress persistence for crash recovery
    - Concurrent processing with worker pools
    - Activity monitoring and rate limit handling
    
    Attributes:
        api_manager (APIManager): Handles API connections and rate limiting
        db_manager (DatabaseManager): Manages database operations
        csv_exporter (CSVExporter): Handles data export to CSV files
        config (Dict): Configuration for data types and fields
        num_workers (int): Number of concurrent worker tasks
        data_type_queue (asyncio.Queue): Queue of data types to process
        retry_manager (RetryManager): Manages operation retries
        message_queue (MessageQueue): Handles progress persistence
        activity_monitor (ActivityMonitor): Tracks processing activity
        finished_data_types (set): Tracks completed data types
    """
    ...
    def __init__(
        ...
        Args:
            api_manager: Manages API connections and rate limiting
            db_manager: Handles database operations
            csv_exporter: Manages CSV file exports
            config_file: Configuration settings for data types
            num_workers: Number of concurrent workers
            retry_manager: Optional custom retry manager
            message_queue: Optional custom message queue
        """
        self.api_manager = api_manager
        self.db_manager = db_manager
        self.csv_exporter = csv_exporter
    ...
    def format_date(date: datetime) -> str:
        """
        Format datetime to standardized string format.
        
        Converts datetime objects to ISO 8601 format with UTC timezone
    ...
    def verify_item_match(self, item: Any, reference_item: Dict[str, Any]) -> bool:
        """
        Verify if an item matches reference data.
        
        Compares item attributes with reference data for:
        - Identity verification
        - Duplicate detection
        - Resume point location
        
        Args:
            item: Item to verify
            reference_item: Reference data for comparison
        ...
        if not reference_item:
            return False
        return all(getattr(item, k, None) == v 
                  for k, v in reference_item.items())

    def extract_item_date(self, item: Any) -> datetime:
        """
        Extract the most relevant date from an item.
        
        Attempts to extract date information in priority order:
        1. Updated date
        2. Action date
        3. Creation date
        ...
        Args:
            item: Item to extract date from
            
        Returns:
            datetime: Extracted date with timezone information
        """
        date_fields = ["updated_at", "updateDate", "update_date", "latest_action_date"]
        for field in date_fields:
            date_str = getattr(item, field, None)
            if date_str:
                try:
                    ...
                    logger.warning(
                        f"Date parsing error for '{date_str}' in '{field}': {str(e)}"
                    )

        return datetime.now(timezone.utc)

    def compute_date_range(self, data_type: str, last_processed_date: str) -> Tuple[Dict[str, Any], datetime]:
        """
        Compute the date range for data extraction.
        
        Determines the appropriate date range for data extraction based on:
        1. Last processed date
        2. Overlap window for data consistency
        3. Current time
        
        Args:
            data_type: Type of data for date range calculation
            last_processed_date: Last recorded processing date
            
        Returns:
            Tuple containing:
            - Dict with date range parameters
            - DateTime of the latest date in range
        """
        end_date = datetime.now(tz=timezone.utc)
        overlap_window = timedelta(hours=1)
        ...
        if last_processed_date:
            try:
                ...
                logger.error(f"Date parsing error for {data_type}: {e}")

        latest_date = start_date if start_date else datetime.min.replace(tzinfo=timezone.utc)
        ...
        return kwargs, latest_date
    
    # =========== Progress Tracking Methods ===========

    def persist_progress_state(self, data_type: str, kwargs: Dict[str, Any], 
                             start_date: datetime, latest_date: datetime, 
                             resume_state: Dict[str, Any]):
        """
        Persist current processing state for potential resume operations.
        
        Saves comprehensive state information including:
        ...
        Args:
            ...
            latest_date: Latest processed date
            resume_state: Current processing state
        """
        message = {
            'kwargs': kwargs,
            'start_date': start_date.isoformat() if start_date else None,
            'overall_latest_date': latest_date.isoformat(),
            'resume_state': resume_state
        }
        self.message_queue.push(data_type, message)
        logging.info(f"Persisted progress state for {data_type}")

    def track_item_progress(self, data_type: str, index: int, total: int):
        """
        Track progress of individual item processing.
        
        Maintains granular progress tracking at the item level for:
        - Progress reporting
        - Resume capability
        - Status monitoring
        ...
        self.message_queue.push(data_type, message)
        logging.debug(f"Tracked progress for {data_type}: item {index + 1}/{total}")

    def track_nested_field_progress(self, data_type: str, item_index: int, 
                                  item_total: int, field_index: int, field_total: int):
        """
        Track progress of nested field processing.
        
        Maintains state for nested field processing including:
        - Parent item position
        - Field processing position
        - Overall completion status
        
        Args:
            data_type: Type of data being processed
            item_index: Current item position
            item_total: Total items
            field_index: Current field position
            field_total: Total fields
        """
        message = {
            'type': 'nested_field',
            'item_index': item_index,
        ...
        self.message_queue.push(data_type, message)
        logging.debug(
            f"Tracked {data_type}: item {item_index + 1}/{item_total}, "
            f"nested field {field_index + 1}/{field_total}"
        )

    def track_relation_progress(self, data_type: str, item_index: int, 
                              item_total: int, relation_index: int, 
                              relation_total: int, processed_count: int):
        """
        Track progress of related entity processing.
        
        Maintains comprehensive state for relationship processing:
        - Parent item position
        - Relationship processing position
        - Count of processed related items
        
        Args:
            data_type: Type of data being processed
            item_index: Current item position
            item_total: Total items
            relation_index: Current relation position
            relation_total: Total relations
            processed_count: Number of processed related items
        """
        message = {
            'type': 'related_item',
            'item_index': item_index,
            'item_total': item_total,
            'related_index': relation_index,
            'related_total': relation_total,
            'related_items_count': processed_count
        }
        self.message_queue.push(data_type, message)
        logging.debug(
            f"Tracked {data_type}: item {item_index + 1}/{item_total}, "
            f"relation {relation_index + 1}/{relation_total}, "
            f"processed: {processed_count}"
        )

    # =========== Data Transformation Methods ===========

    async def transform_single_item(self, item: Any, fields: List[str] = []) -> Dict[str, Any]:
        """
        Transform a single data item into a standardized dictionary format.
        
        This method handles the conversion of various data types into a consistent
        format for storage and export. It manages:
        1. Basic data type handling
        2. Complex object attribute extraction
        3. Date formatting
        4. Type conversion and validation
        
        Args:
            item: The item to transform
            fields: List of fields to include in transformation
            
        Returns:
            Dict[str, Any]: Transformed item in standardized format
        """
        try:
            if isinstance(item, (str, int, float, bool)):
                return {"value": item}
            ...
            logger.error(f"Item transformation error: {str(e)}", exc_info=True)
            return {}

    async def transform_data_batch(self, data_type: str, items_batch: List[Any], 
                                 resume_state: Optional[Dict[str, Any]] = None):
        """
        Transform a batch of items for a specific data type.
        
        This method implements the Transform phase of ETL:
        1. Processes each item in the batch
        2. Handles nested relationships
        3. Extracts related entities
        4. Maintains state for resumability
        
        Args:
            data_type: Type of data being transformed
            items_batch: List of items to transform
            resume_state: Optional state for resuming interrupted processing
        """
        async def process():
            nonlocal resume_state
            logger.info(f"Transforming batch of {len(items_batch)} items for {data_type}")
            fields = self.config.get(data_type, {}).get('fields', [])
            processed_items = []
            ...
            for index in range(start_index, len(items_batch)):
                try:
                    item = items_batch[index]
                    processed_item = await self.transform_single_item(item, fields)
                    processed_items.append(processed_item)

                    self.track_item_progress(data_type, index, len(items_batch))
                    self.message_queue.update_last_exported_item(data_type, processed_item)

                    await self.extract_nested_relations(data_type, item, index, len(items_batch), resume_state)
                    await self.extract_related_entities(data_type, item, index, len(items_batch), resume_state)

                    resume_state = {}
                except Exception as e:
                    logger.error(f"Error transforming item {index + 1}: {str(e)}", exc_info=True)
                    resume_state = {
                        'type': 'item',
                        'item_index': index
                    }
            ...
            await self.csv_exporter.export_to_csv(data_type, processed_items)
            logger.info(f"Completed batch transformation for {data_type}")

        try:
            await self.retry_manager.retry_with_backoff(process)
        except Exception as e:
            logger.error(f"Batch transformation failed for {data_type}: {str(e)}", exc_info=True)
            self.persist_progress_state(data_type, {}, None, None, resume_state)
            raise
        
        
    # =========== Data Extraction Methods ===========

    async def handle_data_type_extraction(self, data_type: str):
        """
        Manage the extraction process for a specific data type.
        
        This method coordinates the complete extraction process for a data type:
        1. Checks for previously exported data
        2. Initializes or resumes extraction
        3. Manages the extraction stream
        4. Handles completion and cleanup
        
        Args:
            data_type: The type of data to extract (e.g., 'bills', 'committees')
        """
        logger.info(f"Starting extraction for {data_type}")

        last_exported_item = await self.validate_data_consistency(data_type)
        if last_exported_item:
            logger.info(f"Resuming {data_type} from {last_exported_item}")
        else:
            logger.info(f"Starting new extraction for {data_type}")

        while True:
            try:
                await self.extract_and_process_data_stream(data_type, last_exported_item)
                self.activity_monitor.update_activity()
                
                self.message_queue.push(data_type, {"status": "FINISHED"})
                logger.info(f"Completed extraction of {data_type}")
                
                self.finished_data_types.add(data_type)
                await self.redistribute_keys(data_type)
                break
            except asyncio.TimeoutError:
                logger.error(f"Timeout during {data_type} extraction. Saving progress.")
                await self.save_global_progress()
                await asyncio.sleep(60)
            except Exception as e:
                logger.error(f"Error processing {data_type}: {str(e)}", exc_info=True)
                await self.db_manager.write_error_to_database(
                    url=f"extraction_{data_type}",
                    error_message=str(e),
                    data_type=data_type
                )
                await asyncio.sleep(60)

    async def extract_and_process_data_stream(self, data_type: str, last_exported_item: Optional[Dict[str, Any]]):
        """
        Extract and process a continuous stream of data for a given type.
        
        This method handles the core extraction logic:
        1. Retrieves or creates processing state
        2. Sets up data streaming from the API
        3. Manages batch processing and pagination
        4. Tracks progress and handles interruptions
        
        Args:
            data_type: Type of data being processed
            last_exported_item: Previously exported item for resumption
        """
        message = self.message_queue.pop(data_type)
        if message:
            logging.info(f"Resuming stream for {data_type} from saved state")
            kwargs = message.get('kwargs', {})
            start_date = datetime.fromisoformat(message.get('start_date', datetime.min.replace(tzinfo=timezone.utc).isoformat()))
            overall_latest_date = datetime.fromisoformat(message.get('overall_latest_date', start_date.isoformat()))
            resume_state = message.get('resume_state', {})
            ...
            last_processed_date = await self.db_manager.get_last_processed_date(data_type)
            kwargs, start_date = await self.compute_date_range(data_type, last_processed_date)
            overall_latest_date = start_date if start_date else datetime.min.replace(tzinfo=timezone.utc)
            resume_state = {}
        ...
        try:
            first_batch = await self.fetch_data_batch_with_backoff(data_generator)
            if isinstance(first_batch, ErrorResult):
                logging.error(f"Error retrieving initial batch for {data_type}: {first_batch.error_message}")
                return
            ...
            if first_batch and first_batch[0]._pagination.get('count'):
                ...
                if not found_last_item:
                    should_process = False
                    for item in first_batch:
                        if self.verify_item_match(item, last_exported_item):
                            found_last_item = True
                            should_process = True
                            break
                ...
                if should_process:
                    try:
                        await self.transform_data_batch(data_type, first_batch, resume_state)
                        resume_state = {}
                    except asyncio.TimeoutError:
                        logging.error(f"Timeout occurred while processing batch for {data_type}")
                        self.persist_progress_state(data_type, kwargs, start_date, overall_latest_date, resume_state)
                        raise
        ...
        while total_processed < target_count:
            try:
                items_batch = await self.fetch_data_batch_with_backoff(data_generator)
            except asyncio.TimeoutError:
                logging.error(f"Timeout occurred while fetching batch for {data_type}")
                self.persist_progress_state(data_type, kwargs, start_date, overall_latest_date, resume_state)
                raise
            ...
            if items_batch:
                ...
                try:
                    await self.transform_data_batch(data_type, items_batch, resume_state)
                    resume_state = {}
                except asyncio.TimeoutError:
                    logging.error(f"Timeout occurred while processing batch for {data_type}")
                    self.persist_progress_state(data_type, kwargs, start_date, overall_latest_date, resume_state)
                ...
                batch_latest_date = max((self.extract_item_date(item) for item in items_batch), 
...
# =========== Relationship Processing Methods ===========
    ...
    async def extract_nested_relations(self, data_type: str, item: Any, index: int, total: int, resume_state: Dict[str, Any]):
        """
        Extract and process nested relationships within an item.
        
        This method handles nested data structures:
        1. Identifies nested fields from configuration
        2. Extracts nested data maintaining parent-child relationships
        3. Transforms nested data while preserving context
        
        Args:
            data_type: Parent data type being processed
            item: Parent item containing nested data
            index: Current item index
            total: Total items in batch
            resume_state: State for resuming interrupted processing
        """
        logger.info(f"Processing nested fields for {data_type} (item {index + 1}/{total})")
        nested_fields = self.config.get(data_type, {}).get('nested_fields', [])
        logger.debug(f"Nested fields for {data_type}: {nested_fields}")
        ...
        for field_index in range(start_field_index, len(nested_fields)):
            ...
            if nested_data:
                logger.info(f"Found nested data for {field_name}")
                await self.transform_nested_field(data_type, field_name, nested_data, item, field_config)
            else:
                logger.debug(f"No nested data found for {field_name}")

            self.track_nested_field_progress(data_type, index, total, field_index, len(nested_fields))

    async def transform_nested_field(self, data_type: str, nested_field: str, nested_data: Any, parent_item: Any = None, nested_config: Dict = None):
        """
        Transform and process a single nested field.
        
        Handles different types of nested data:
        1. Lists of primitive types
        2. Lists of objects
        3. Single objects
        4. Primitive types
        
        Args:
            data_type: Parent data type
            nested_field: Name of the nested field
            nested_data: The nested data to process
            parent_item: Parent item containing the nested data
            nested_config: Configuration for nested field processing
        """
        nested_data_type = f"{data_type}_{nested_field}"
        id_fields = self.config[data_type].get('id_fields', ['id'])
        ...
        if isinstance(nested_data, list):
            ...
            if nested_config and 'fields' in nested_config:
                formatted_data = [add_missing_id_fields({field: nested_data.get(field) 
    ...
    async def extract_related_entities(self, data_type: str, item: Any, index: int, total: int, resume_state: Dict[str, Any]):
        """
        Extract and process related entities for an item.
        
        Handles relationships between different entity types:
        1. Identifies related entities from configuration
        2. Extracts related data while maintaining relationships
        3. Manages secondary relationships
        4. Preserves referential integrity
        
        Args:
            data_type: Primary data type being processed
            item: Source item for relationship extraction
            index: Current item index
            total: Total items in batch
            resume_state: State for resuming interrupted processing
        """
                ...
                try:
                    ...
                    async for related_item in async_generator:
                        ...
                        if isinstance(related_item, ErrorResult):
                            ...
                            logger.error(f"Error retrieving related data for {data_type}_{related_type}: {error_message}")
                            await self.db_manager.write_error_to_database(
                                getattr(related_item, 'url', 'unknown'),
                                error_message,
                                data_type=f"{data_type}_{related_type}"
                            )
                            break

                        if related_item:
                            processed_item = await self.transform_single_item(related_item)
                            if processed_item:
                                for id_field in id_fields:
                                    if id_field not in processed_item:
                                        parent_id_value = getattr(item, id_field, None)
                            ...
                            await self.handle_secondary_relations(data_type, related_type, related_item)

                        self.track_relation_progress(
                            data_type, index, total, 
                            related_index, len(related_types), 
                            len(related_items)
                        )
    ...
    async def handle_secondary_relations(self, main_data_type: str, related_type: str, related_item: Any):
        """
        Process secondary level relationships.
        
        Handles second-level relationships and nested fields:
        1. Processes relationships of related items
        2. Handles nested fields within related items
        3. Maintains relationship hierarchy
        
        Args:
            main_data_type: Primary data type
            related_type: Type of the related entity
            related_item: Related item to process
        """
        logger.debug(f"Processing second level related data for {main_data_type}_{related_type}")
        ...
        # Process second-level related types
        for second_level_type in second_level_related_types:
            self.activity_monitor.update_activity()
            method_name = f"get_{second_level_type}"
            ...
            if hasattr(related_item, method_name):
                try:
                    ...
                    async for second_level_item in async_generator:
                        ...
                        if isinstance(second_level_item, ErrorResult):
                            error_message = second_level_item.error_message or second_level_item.data.get("error", "Unknown error")
                            logger.error(f"Error retrieving second level data for {main_data_type}_{related_type}_{second_level_type}: {error_message}")
                            await self.db_manager.write_error_to_database(
                                getattr(second_level_item, 'url', 'unknown'),
                                error_message,
                                data_type=f"{main_data_type}_{related_type}_{second_level_type}"
                            )
                            break
                            
                        if second_level_item:
                            processed_item = await self.transform_single_item(second_level_item)
                            if processed_item:
                                for id_field in id_fields:
                                    if id_field not in processed_item:
                                        id_value = getattr(related_item, id_field, None)
                                        if id_value is not None:
                                            processed_item[id_field] = id_value
                                second_level_items.append(processed_item)

                    if second_level_items:
                        await self.csv_exporter.export_to_csv(
                            f"{main_data_type}_{related_type}_{second_level_type}",
                            second_level_items
                        )
                        
                except Exception as e:
                    logger.error(f"Error processing second level data {second_level_type}: {str(e)}", exc_info=True)
                    await self.db_manager.write_error_to_database(
                        getattr(related_item, 'url', 'unknown'),
                        str(e),
                        data_type=f"{main_data_type}_{related_type}_{second_level_type}"
        ...
        # Process second-level nested fields
        for nested_field in second_level_nested_fields:
            self.activity_monitor.update_activity()
            if isinstance(nested_field, dict):
                field_name = next(iter(nested_field))
            ...
            await self.transform_nested_field(
                f"{main_data_type}_{related_type}",
                field_name,
                getattr(related_item, field_name, None),
                related_item,
                nested_config
            )
    ...
    async def process_all_data(self):
        """
        Process all configured data types concurrently.
        
        Orchestrates the concurrent processing of multiple data types by:
        1. Managing worker tasks
        2. Handling API key availability
        3. Monitoring progress
        4. Managing process interruption
        """
        ...
        for data_type in main_data_types:
            message = self.message_queue.pop(data_type)
            if message and message.get("status") == "FINISHED":
                logger.info(f"Skipping {data_type} - already processed")
                continue
        ...
        try:
            while not self.data_type_queue.empty() or any(not w.done() for w in workers):
                ...
                if not active_keys:
                    sleep_time = min(status["sleep_until"] for status in self.api_manager.api_key_status.values() if status["sleeping"])
                    wait_time = (sleep_time - datetime.now(timezone.utc)).total_seconds()
                    logger.info(f"All API keys sleeping. Waiting {wait_time} seconds")
                    self.activity_monitor.start_sleep()
                    await asyncio.sleep(wait_time)
                    self.activity_monitor.end_sleep()
                else:
    ...
    async def worker(self):
        """
        Worker coroutine that processes data types from the queue.
        
        Each worker:
        1. Takes data types from the queue
        2. Processes them independently
        3. Handles errors and logging
        4. Continues until all work is done or canceled
        """
        while True:
            try:
                data_type = await self.data_type_queue.get()
                logger.info(f"Worker starting to process {data_type}")
                await self.handle_data_type_extraction(data_type)
                logger.info(f"Worker finished processing {data_type}")
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    async def validate_data_consistency(self, data_type: str) -> Optional[Dict[str, Any]]:
        """
        Validate consistency between stored state and CSV data.
        
        Checks for:
        1. Last exported item in message queue
        2. Corresponding data in CSV files
        3. Data consistency between sources
        
        Args:
            data_type: Type of data to validate
            
        Returns:
            Optional[Dict[str, Any]]: Last exported item if validated, None otherwise
        """
        last_exported_item = self.message_queue.get_last_exported_item(data_type)
        if not last_exported_item:
            return None
    ...
    async def save_global_progress(self):
        ...
        Ensures that progress is not lost in case of:
        1. Process interruption
        2. System shutdown
        3. Error conditions
        
        Enables process resumption from last known good state.
        """
        for data_type in self.get_main_data_types():
            message = self.message_queue.pop(data_type)
            if message:
    ...
    async def redistribute_keys(self, finished_data_type: str):
        ...
        Optimizes API key usage by:
        1. Reclaiming keys from completed processes
        2. Redistributing to active processes
        3. Maintaining optimal key distribution
        
        Args:
            finished_data_type: Data type that has completed processing
        """
        finished_keys = self.api_manager.congress_instances[finished_data_type]._adapter.api_keys
        self.dynamic_key_pool.extend(finished_keys)
    ...
    async def run(self):
        ...
        1. Initializes resources and connections
        2. Processes all configured data types
        3. Handles interruptions gracefully
        4. Ensures proper cleanup
        
        The process can be resumed from the last saved state if interrupted.
        """
        try:
            await self.initialize()
            try:
                ...
                logger.info("Received interrupt. Saving state and stopping gracefully.")
                await self.save_global_progress()
        except Exception as e:
            logger.error(f"ETL process error: {str(e)}", exc_info=True)
        finally:
            await self.cleanup()

    async def cleanup(self):
        """
        Perform cleanup operations for the ETL process.
        
        Ensures proper resource cleanup:
        1. Closes API sessions
        2. Closes database connections
        3. Saves final state
        4. Closes message queue
        """
...
async def main():
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logger = logging.getLogger(__name__)
    config_file = "congressional_config.yaml"
